...
def post_process(model_output, threshold=0.5, return_seg_onehot_cluster=False, need_erosion_or_expansion=False,
                 kernel_size=3, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
...
def img_3d_erosion_or_expansion(img_3d, kernel_size=3,
...
def fill_inner_hole(seg_onehot_final, need_erosion_or_expansion=False, kernel_size=3,
                    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
    seg_onehot_background = np.array(seg_onehot_final == 0, dtype=int)
    ...
    bg = np.array(labels_bg == unique_vals[sort_locs][0], dtype=int)
...
def get_super_vox(seg_onehot, connectivity=2, offset=[1, 1, 1]):
    ...
    seg_onehot_super_vox = watershed(-seg_onehot_edt, mask=np.array(seg_onehot > 0), connectivity=connectivity,
                                     offset=offset)
...
class Cluster_super_vox():
    ...
    def fit(self, input_3d_img, restrict_area_3d=None):
        ...
        if restrict_area_3d is None:
            self.restrict_area_3d = np.array(input_3d_img == 0, dtype=int)
        else:
    ...
    def get_outlayer_area(self, current_val):
        ...
        current_crop_img_onehot = np.array(current_crop_img == current_val, dtype=int)
    ...
    def growCluster(self, valid_neighbor_vals, current_val):
        ...
        if len(valid_neighbor_vals) > 0:
            ...
            print('Assign label ' + str(current_val) + ' to current val\'s neighbors: ' + str(new_valid_neighbor_vals),
                  end="\r")
...
def get_outlayer_of_a_3d_shape(a_3d_shape_onehot, layer_thickness=1):
    ...
    outlayer[1:shape[0], :, :] += np.array(a_3d_crop_diff_x1 == 1, dtype=int)
    outlayer[0:shape[0] - 1, :, :] += np.array(a_3d_crop_diff_x2 == 1, dtype=int)
    outlayer[:, 1:shape[1], :] += np.array(a_3d_crop_diff_y1 == 1, dtype= int)
    outlayer[:, 0:shape[1] - 1, :] += np.array(a_3d_crop_diff_y2 == 1, dtype= int)
    outlayer[:, :, 1:shape[2]] += np.array(a_3d_crop_diff_z1 == 1, dtype= int)
    outlayer[:, :, 0:shape[2] - 1] += np.array(a_3d_crop_diff_z2 == 1, dtype= int)

    outlayer = np.array(outlayer > 0, dtype= int)
...
def add_broken_parts_to_the_result(connection_dict, model_output_prob_map, seg_processed_onehot, threshold=0.5,
    ...
    for idx, item in enumerate(center_map_end_point_dict.keys()):
        ...
        model_output_prob_map_crop, crop_coord = get_crop(model_output_prob_map, search_center,
        ...
        model_output_crop_revised = np.array(model_output_crop_delete_current_seg > current_threshold, dtype=int)
    ...
    seg_processed_onehot_II = np.array(seg_processed_onehot_II > 0, dtype=int)
