...
def exp_rampdown(rampdown_length, num_epochs, lr_step, lr_decay):
    ...
    def warpper(epoch):
        if epoch >= (num_epochs - rampdown_length):
            n = epoch // lr_step
            return np.power(lr_decay, n)
        else:
            return 1.0
    return warpper

def exp_warmup(rampup_length, rampdown_length, num_epochs, lr_step, lr_decay):
    rampup = exp_rampup(rampup_length)
    rampdown = exp_rampdown(rampdown_length, num_epochs, lr_step, lr_decay)
    def warpper(epoch):
...
def load_pickle(filename='./split_dataset_not673_1_2.pickle'):
    """
...
def save_pickle(dict, filename='data.pickle'):
    """
...
def specificity_np(y_pred,y_true):
    """
    :param y_pred: prediction
    :param y_true: target ground-truth
    :return: sensitivity
