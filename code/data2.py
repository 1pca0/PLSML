...
class semi_train_dataset(AirwayData):
	...
	def __init__(self, config, args, split_comber=None, debug=False):
		...
		for i in unlabel_data_file_names:
			...
			tem_2 = 'mask'+'/'+tem
			self.unlabeled_name.append(tem)
		...
		for raw_path in data_file_names:
			...
			b = temp.split('/')[-1]
			c = a+'/'+b
			raw_path = os.path.join(self.datapath,c)
			print(raw_path)
			label_c = 'mask' +'/'+b
			label_path  = os.path.join(self.datapath,label_c)
...
class val_dataset(AirwayData):
	...
	def __init__(self, config, args, split_comber=None, debug=False):
		...
		for raw_path in data_file_names:
			...
			# label_path = raw_path.replace('clean_hu', 'label')
			# assert (os.path.exists(label_path) is True)
			temp = raw_path#D:/code/Cerebral-Artery-Segmentation-Challenge-MICCAI-SMRA-2023-main/CAS2023_traindaataset/data/000.nii.gz'
			a = temp.split('/')[-2]
			b = temp.split('/')[-1]
			c = a+'/'+b
			raw_path = os.path.join(self.datapath, c)
