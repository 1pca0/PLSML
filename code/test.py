...
def calculate_ahd(pred,label):
    # pred_data = nibabel.get_fdata()
    # label_data = nibabel.get_fdata()
    a_to_b = __surface_distances(pred,label)
    b_to_a = __surface_distances(label,pred)
    ahd = (np.mean(a_to_b) + np.mean(b_to_a)) / 2
    return ahd
def calculate_fpr_tpr(y_pred, y_true, alpha=0.25, gamma=2.0, threshold=0.5):
    """
    Calculate False Positive Rate (FPR) and True Positive Rate (TPR) for binary classification.
    ...
    :param threshold: decision threshold for converting probabilities to binary predictions, default=0.5
    :return: FPR, TPR
    """
    ...
    TN = np.sum((1 - y_pred_f) * (1 - y_true_f))
    FP = np.sum(y_pred_f * (1 - y_true_f))
    FN = np.sum((1 - y_pred_f) * y_true_f)
    eps = 1e-6
    # tn = np.sum(~y_pred_f & ~y_true_f)
    # fp = np.sum(y_pred_f & ~y_true_f)
    ...
    return fpr, tpr,dsc
def evaluation_1(data_path, parsing_path,logger,dilate =False):
    ...
    for i in range(len(file_list)//n):
        ...
        pred = nibabel.load(os.path.join(data_path, file_list[n*i+1]))

        label = label.get_fdata()
        pred2 = pred.get_fdata()

        # parsing = parsing.get_fdata()

        #dice = metric.binary.dc(pred2, label)  # Dice系数
        jc = metric.binary.jc(pred2,label)  # Jaccard指数
        hd = metric.binary.hd95(pred2,label)  # Hausdorff距离
        asd = metric.binary.asd(pred2, label)  # ASD


        ahd = calculate_ahd(pred2,label)

        AHD.append(ahd)

        fpr, tpr,dsc = calculate_fpr_tpr(pred2,label)
        ...
        TPR.append(tpr)
        DSC.append(dsc)
        print(file_list[n * i].split('-')[0], "jc: %0.4f" % (jc), "hd: %0.4f" % (hd),
              "asd: %0.4f" % (asd), "fpr: %0.6f" % (fpr), "tpr: %0.4f" % (tpr),"dsc:%0.4f"%(dsc),"ahd:%0.4f"%(ahd))
        ...
        # print(file_list[n*i].split('_')[0], "Length: %0.4f"%(sen), "Precision: %0.4f"%(pre), "Branch: %0.4f"%(branch), "Dice: %0.4f"%(curdice), "Se: %0.4f"%(cursensi))
        print(file_list[n * i].split('_')[0], "Dice: %0.4f" % (curdice), "Se: %0.4f" % (cursensi))
    ...
    ahd_mean = np.mean(AHD)

    if logger is not None :
        logger.info(f'dice_mean: {dice_mean:.4f}')
        logger.info(f'jc_mean: {jc_mean:.4f}')
        logger.info(f'hd_mean: {hd_mean:.4f}')
        logger.info(f'asd_mean: {asd_mean:.4f}')
        logger.info(f'fpr_mean: {fpr_mean:.4f}')
        logger.info(f'tpr_mean: {tpr_mean:.4f}')
        logger.info(f'dsc_mean: {dsc_mean:.4f}')
        logger.info(f'se_mean: {se_mean:.4f}')
        logger.info(f'ahd_mean: {ahd_mean:.4f}')
        logger.info('\n')


    print("dice_mean:%0.4f" % (dice_mean), "jc_mean:%0.4f" % (jc_mean), "hd_mean: %0.4f" % (hd_mean),"asd_mean:%0.4f" % (asd_mean),
          "fpr_mean:%0.6f" % (fpr_mean), "tpr_mean:%0.4f" % (tpr_mean),"dsc_mean:%0.4f"%(dsc_mean),"se:%0.4f"%(se_mean),"ahd:%0.4f"%(ahd_mean))
...
def FPR_np(y_pred, y_true):
    ...
    # intersection = np.sum(y_true_f * y_pred_f)
    return FPR,TPR
def evaluation_2(data_path, parsing_path):
    logging.basicConfig(filename= r"2023_1-7_new_way_mean_teacher",level=logging.INFO)
